<?php
    require_once 'topbar.php';
    require_once 'button.php';
?>

<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Glow | Careers</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com/" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Pacifico&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="styles/style.css">
</head>
<body>
    <?php
        echo createTopbar(
            TopbarVariant::SINGULAR, 
            array(
                new MenuOption('./styles/images/suitcase_fill.svg', IconSize::Normal, 'Jobs', 'jobs.php'),
                new MenuOption('./styles/images/folder_open_fill.svg', IconSize::Normal, 'Apply', 'apply.php'),
                new MenuOption('./styles/images/emoji_fill.svg', IconSize::Normal, 'About', 'about.php')
            ), 
            './styles/images/glow-logo.svg', 'Jobs'
        )
    ?>
    <section id="discover-wrapper">
        <div id="discover-contents">
            <h1>Discover <span class="glow-fancy-text">Glow</span></h1>
            <h6>Leave your impact on the world</h6>
             <?php echo createButton(ButtonSize::Normal, ButtonVariant::Filled, ButtonColor::Amber, '', 'Start now', 'button', '#open-positions-wrapper') ?>

        </div>
        <div id="glow-fancy-wrapper">
            <svg id="glow-fancy" viewBox="0 0 1069 665" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M279.683 157.407C279.683 157.407 310.454 85.5425 249.843 61.2982C205.504 43.5625 129.921 58.2425 85.2609 107.922C40.7759 157.407 18.4956 222.477 32.1098 290.921C49.1275 376.475 107.174 444.779 289.706 396.057C289.706 396.057 301.647 287.406 299.111 283.937C294.863 278.127 212.378 278.771 215.575 281.596C218.771 284.421 296.411 279.957 297.932 284.421C299.453 288.885 289.196 404.813 289.196 404.813C278.226 476.932 247.308 642.795 189.732 638.969C124.704 634.647 135.757 531.762 242.11 463.91C348.462 396.057 406.949 377.182 467.758 281.596C517.187 203.898 552.439 128.334 538.418 56.6257C532.631 27.0271 496.581 18.3422 475.523 39.3996C457.897 57.0262 444.694 80.5713 425.678 147.351C392.362 264.345 405.123 395.738 463.689 412.929C524.091 421.58 558.774 310.435 563.787 310.435C568.8 310.435 554.128 414.948 618.63 417.137C670.962 418.914 721.132 360.585 686.367 270.814C672.844 235.896 631.027 223.267 606.618 248.472C586.928 268.804 592.65 288.065 606.618 296.002C648.254 319.66 693.368 329.712 742.872 310.435C790.55 291.87 790.164 248.472 794.505 248.472C798.846 248.472 782.03 414.649 815.884 419.508C832.831 421.94 902.371 252.678 906.785 253.387C911.199 254.096 913.05 427.709 934.048 419.508C960.7 409.098 1034.07 280.929 1042.15 251.282" stroke="#FADCA6" stroke-width="50" stroke-linecap="round"/>
            </svg> 
        </div>
       <iframe id ="jobs-effect" src="data:text/html;base64,"></iframe>
    </section>

    <!-- This section's text was mostly generated by ChatGPT. NOT THE HTML, THE TEXT!!! -->
    <section id="open-positions-wrapper">
        <div id="open-positions-header">
            <h3>Open Positions</h3>
            <aside>Discover 2 job positions <br> that are currently open.</aside>
        </div>

        <div class="job-position">
            <div class="job-position-icon-wrapper">
                <img src="images/icons/database.svg" alt="Network Administrator Icon">
            </div>

            <div class="job-contents">
                <div class="job-contents-header">
                    <h5>Network Administrator</h5>
                    <p>Specialise in designing, implementing, maintaining, and troubleshooting computer networks, ensuring they function smoothly and securely.</p>
                </div>

                <div>
                    <h6>Position Overview</h6>
                    <p>A Network Administrator is responsible for managing Glow’s computer networks, including local area networks (LANs), wide area networks (WANs), intranets, and internet systems. They ensure that network infrastructure is robust and operates without disruption.</p>
                </div>
                <div>
                    <h6>Salary and Benefits</h6>
                    <p>$231K - $340K annually, with the possibility of additional equity-based incentives.</p>
                </div>
                <div>
                    <h6>Reporting Structure</h6>
                    <p>The Network Administrator will report to the IT Manager.</p>
                </div>
                <div>
                    <h6>Key Responsibilities</h6>
                    <ul>
                        <li>Maintain and monitor LANs, WANs, and intranet systems.</li>
                        <li>Apply security measures including firewall rules, patches, and encryption protocols.</li>
                        <li>Upgrade systems and network infrastructure as needed.</li>
                        <li>Provide end-user technical support and network troubleshooting.</li>
                        <li>Implement and manage backup and disaster recovery plans.</li>
                        <li>Document network policies, architecture, and configurations.</li>
                    </ul>
                </div>
                <div>
                    <h6>Essential Skills and Qualifications</h6>
                    <ol>
                        <li>Bachelor’s degree in IT, Computer Science, or related field.</li>
                        <li>CCNA or CompTIA Network+ certification.</li>
                        <li>Strong command of networking protocols (TCP/IP, DNS, DHCP).</li>
                        <li>Experience with AWS, Azure, or Google Cloud networking services.</li>
                        <li>Excellent problem-solving and analytical skills.</li>
                    </ol>
                </div>
                <div class="job-position-bottom-wrapper">
                    <p>NET01</p>
                    <?php echo createButton(ButtonSize::Normal, ButtonVariant::Filled, ButtonColor::Amber, '', 'Apply', 'button', 'apply.php') ?>
                </div>
            </div>
        </div>
        <div class="job-position">
            <div class="job-position-icon-wrapper">
                <img src="images/icons/brain.svg" alt="Machine Learning Engineer Icon">
            </div>

            <div class="job-contents">
                <div class="job-contents-header">
                    <h5>Machine Learning Engineer</h5>
                    <p>Engineer, develop and implement machine learning algorithms into Glow's ecosystem.</p>
                </div>

                <div>
                    <h6>Position Overview</h6>
                    <p>A Machine Learning Engineer is responsible for designing, developing, and deploying intelligent
                        algorithms and predictive models that allow software systems to learn and make decisions. This role
                        bridges the gap between data science and software engineering, playing a crucial part in creating scalable
                        Al solutions that solve real-world problems.</p>
                </div>
                <div>
                    <h6>Salary and Benefits</h6>
                    <p>$330,000 - $670,000 annually, including performance bonuses.</p>
                </div>
                <div>
                    <h6>Reporting Structure</h6>
                    <p>Lead Al Engineer or Head of Data Science.</p>
                </div>
                <div>
                    <h6>Key Responsibilities</h6>
                    <ul>
                        <li>Design, train, and evaluate machine learning models for classification, prediction, and automation tasks.</li>
                        <li>Collaborate with data scientists, engineers, and product teams to integrate ML into products.</li>
                        <li>Build data pipelines and preprocess large datasets for modeling.</li>
                        <li>Optimize models for accuracy, efficiency, and scalability.</li>
                        <li>Deploy models using MLOps tools and monitor their performance in production environments.</li>
                        <li>Document processes, experiments, and outcomes for reproducibility.</li>
                    </ul>
                </div>
                <div>
                    <h6>Essential Skills and Qualifications</h6>
                    <ol>
                        <li>Bachelor's or Master's degree in Computer Science, Artificial Intelligence, or related field.</li>
                        <li>Proficiency in Python and ML libraries such as TensorFlow, PyTorch, Scikit-learn.</li>
                        <li>Strong foundation in statistics, data analysis, and linear algebra.</li>
                        <li>Experience with cloud ML platforms (AWS SageMaker, GCP Al Platform, Azure ML).</li>
                        <li>Solid understanding of software development practices and version control systems like Git.</li>
                    </ol>
                </div>
                <div class="job-position-bottom-wrapper">
                    <p>MAC01</p>
                    <?php echo createButton(ButtonSize::Normal, ButtonVariant::Filled, ButtonColor::Amber, '', 'Apply', 'button', 'apply.php') ?>
                </div>
            </div>
        </div>
    </section>

      <!-- footer element. This contains a link to the jira page, couple of menu options and a cool logo animation -->
      <footer>
        <!-- Menu options (Jobs and about) -->
        <div id="footer-menu">
            <a href="jobs.html" class="menu-button">
                <button><img src="images/icons/jobs.svg" alt="Jobs icon">Jobs</button>
            </a>
            <a href="about.html" class="menu-button">
                <button><img src="images/icons/about.svg" alt="About icon">About</button>
            </a>
        </div>

        <a href="index.html" id="glow-logo-footer">
            <img src="images/glow-outlined-footer.svg" alt="">
        </a>

        <!-- Jira button to the jira board -->
        <a href="https://fletcher06.atlassian.net/jira/software/projects/CGAP1/summary" target="_blank">
            <button id="jira-button">Jira board<img src="images/icons/link-out.svg" alt="About icon"></button>
        </a>
    </footer>
</body>
</html>